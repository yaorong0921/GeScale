...
def main():
    ...
    if not args.test:
        ...
        categories, args.test_list, args.root_path, prefix = datasets_video.return_dataset('SHGDTuples', args.modality)
    num_class = len(categories)

    args.store_name = '_'.join([args.dataset, args.modality, args.arch,
                                'segment%d'% args.num_segments])
    print('storing name: ' + args.store_name)

    model = MFF(num_class, args.num_segments, args.modality,
    ...
    if args.pretrained:
        ...
        if os.path.isfile(args.pretrained):
            ...
            #for name, param in pretrained_state_dict.items():
                    ...
                    #print(name)
            pretrained_state_dict = {k:v for k, v in pretrained_state_dict.items() if 'module.consensus.classifier.3.' not in k}
            model_dict = model.state_dict()
            weight_conv_t = pretrained_state_dict[name]

        else:
            print(("=> no pretrained model checkpoint found at '{}'".format(args.pretrained)))
        ...
        print("=> loaded pretrained model checkpoint '{}'".format(args.pretrained))
    print(model)
    ...
    if not args.test:
        ...
        DataSet(args.root_path, args.train_list, num_segments=args.num_segments,
        ...
        val_loader = torch.utils.data.DataLoader(
         DataSet(args.root_path, args.val_list, num_segments=args.num_segments,
                   modality=args.modality,
                   image_tmpl=prefix,
                   dataset=args.dataset,
                   random_shift=False,
        ...
        test_loader = torch.utils.data.DataLoader(
         DataSet(args.root_path, args.test_list, num_segments=args.num_segments,
                   modality=args.modality,
                   image_tmpl=prefix,
                   dataset=args.dataset,
                   random_shift=False,
...
# give the "No gesture/Hand up/Hand down less weight than the other classes. No:4420 Hand up:2280 Hand Down:2190 Others:228
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            prec1 = validate(val_loader, model, criterion, (epoch + 1) * len(train_loader), log_training, num_class)
...
def train(train_loader, model, criterion, optimizer, epoch, log):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        prec1, prec5, correct_mat = accuracy(output.data, target, topk=(1,5))
...
def validate(val_loader, model, criterion, iter, log, num_class):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    conf_mat = torch.zeros(num_class, num_class)
    for i, (input, target) in enumerate(val_loader):
        ...
        #for t,p in zip(target.view(-1), pred_data.view(-1)):
        #    conf_mat[t,p] += 1
