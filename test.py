...
def test(epoch, data_loader, model, arg):
...
##### initial all the errors
    ...
    for i, (inputs, targets, input_length) in enumerate(data_loader):
        ...
        for j in range(0, step+1):
            M = list(range(j * par, min(j*par+arg.num_segments*par, input_length*par)))
            if len(M) < 8*par:
                break
            else:
                input_single = inputs[:,M[0]:(M[0]+arg.num_segments*par),:,:]
    ...
    ## Deactivate Classifier Queue and start Viterbi decoder
        if len(input_viterbi) > 3:
            ...
            if not path_m == target and EoG:
                ...
                classifier_error += (3-sum(1 for a, b in zip(path_m, target) if a == b))
