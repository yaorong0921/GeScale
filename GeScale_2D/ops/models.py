...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 crop_num=1, partial_bn=False, print_spec=True):
        ...
        if new_length is None:
            if modality == "RGB":
                self.new_length = 1
            elif modality == 'GrayDepth' or modality == 'Depth' or modality == 'Gray':
                self.new_length = 1
            elif modality == "Flow":
                self.new_length = 5
    ...
    TSN Configurations:
        ...
        elif self.modality == 'GrayDepth':
            print("Converting the ImageNet model to GrayDepth init model")
            self.base_model = self._construct_flow_model(self.base_model)
            print("Done. GrayDepth model ready.")
        elif self.modality == 'Gray' or self.modality == 'Depth':
            print("Converting the ImageNet model to Depth/Gray init model")
            self.base_model = self._construct_onechannel_model(self.base_model)
            print("Done. Depth/Gray model ready.")
    ...
    def _prepare_base_model(self, base_model):
        ...
        elif 'shufflenet' in base_model:
            self.base_model = shufflenet.get_model()
            # Load pretrained model
            checkpoint = torch.load('shufflenet.pth.tar')
    ...
    def get_optim_policies(self):
        ...
        # return [
        #     {'params': first_conv_weight, 'lr_mult': 0.1, 'decay_mult': 0.1,
        #      'name': "first_conv_weight"},
        #     {'params': first_conv_bias, 'lr_mult': 0.1, 'decay_mult': 0,
        #      'name': "first_conv_bias"},
        #     {'params': normal_weight, 'lr_mult': 0.1, 'decay_mult': 0.1,
        #      'name': "normal_weight"},
        #     {'params': normal_bias, 'lr_mult': 0.1, 'decay_mult': 0,
        #      'name': "normal_bias"},
        #     {'params': bn, 'lr_mult': 0.1, 'decay_mult': 0,
        #      'name': "BN scale/shift"},
        # ]
    ...
    def get_augmentation(self):
        ...
        elif self.modality == 'RGBFlow' or self.modality == 'GrayDepth' or self.modality == 'Depth' or self.modality == 'Gray':
